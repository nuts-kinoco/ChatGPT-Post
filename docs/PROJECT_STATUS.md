...
**Phase 7 — リリース判定と引き渡し: 完了（2026-09-15）。MVP v1.0 引き渡し可能。**
...
| 7 リリース判定 | 完了 | 2026-09-15 | 全 23 エラーコードのシナリオ表、DOM fixture テスト、運用手順 2.0、README、SKILL、Codex 最終レビュー（High）: Critical/High なし、Medium 2 を修正。Unit+fixture 134 件 |
| 6 画像・キュー・追記 | 完了 | 2026-09-15 | 契約 1.2（images[]、newChat:false）、生成画像取得、画像添付、worker、SKILL.md、EX-08 / CX-06。Live 13 件（`docs/live-results/20260915-phase6.md`）。Unit 105 件。Codex: High 2 / Medium 1 / Low 1 を修正、Low 1 を保留 |
...
- `docs/DECISION-LOG.md`（A-001〜A-100）
...
MVP v1.0 として引き渡し。残課題（優先順）:
1. OQ-001: OpenAI 利用規約の手動確認（PO）。R-001 の記述更新
2. 探索の残り: CX-03（大規模 bundle の劣化点）、英語 UI のラベル確認、NSFW 拒否範囲の観測（回避なし）、EX-08 の精度評価（人手正解）
3. ビューア経由の画像ダウンロードは Chrome のクラッシュが解消するまで opt-in のまま
4. `knowledge/` の週次・月次コンパクションの運用開始（22 §5）
5. 「えまきのこ」「たぐきのこ」側の連携: `SKILL.md` の手順とキュー規約で試行し、必要なら `prompts/classify-batch.md` を調整
