...
| 同上 | `VERDICT_CONVERSATION_MISMATCH(cause)` | `FAILED(CONVERSATION_MISMATCH)` | 同上（`error.cause` に理由）。A-116: `observeLoop()` が送信直後に束縛した会話URLと現在のURLの不一致を検出した場合。`judge()` を経由せず直接発行される（別会話の観測値で完了判定させないため） |
